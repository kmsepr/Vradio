...
def play():
    ...
    def generate():
        ...
        try:
            ...
            if f:
                f.close()
            # ✅ Only kill ffmpeg if not recording
            if ffmpeg_process and not recording:
                ffmpeg_process.kill()
...
def record_size():
    global record_file, recording
    if record_file and os.path.exists(record_file) and recording:
